...
# Upgrade system-level packages as root to resolve Trivy security vulnerabilities in /usr/local/
RUN pip install --no-cache-dir --upgrade pip "setuptools>=78.1.1" "wheel>=0.46.2" "jaraco.context>=6.1.0" "msgpack>=1.2.1" \
    && rm -rf /usr/local/lib/python3.10/site-packages/setuptools/_vendor/jaraco.context* \
    && rm -rf /usr/local/lib/python3.10/site-packages/setuptools/_vendor/wheel* \
    && rm -rf /usr/local/lib/python3.10/site-packages/setuptools-*.dist-info \
