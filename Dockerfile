...
# Upgrade system-level python packages as root before creating non-root user
...
RUN pip install --user --no-cache-dir *.whl \
    && pip install --user --no-cache-dir --upgrade pip "setuptools>=78.1.1" "wheel>=0.46.2" "jaraco.context>=6.1.0" "msgpack>=1.2.1" \
    && rm *.whl
