...
# Upgrade system-level python packages to fix Trivy security vulnerability findings in /usr/local/
...
# Install the wheel package locally and upgrade user-level packages in single pass
RUN pip install --user --no-cache-dir --upgrade pip "setuptools>=78.1.1" "wheel>=0.46.2" "jaraco.context>=6.1.0" "msgpack>=1.2.1" *.whl \
    && rm *.whl
