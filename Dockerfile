...
LABEL org.opencontainers.image.description="High-Performance Network Digital Twin & Pre-Flight Validation Platform (nroute)"
LABEL org.opencontainers.image.licenses="MIT"

WORKDIR /app

# Upgrade system level packages to resolve security vulnerabilities and purge old dist-info
RUN pip install --no-cache-dir --upgrade pip "setuptools>=78.1.1" "wheel>=0.46.2" "jaraco.context>=6.1.0" "msgpack>=1.2.1" \
    && find /usr/local/lib/python3.10/site-packages -maxdepth 1 -type d -name "setuptools-70*" -exec rm -rf {} + \
    && find /usr/local/lib/python3.10/site-packages -maxdepth 1 -type d -name "wheel-0.45*" -exec rm -rf {} + \
    && find /usr/local/lib/python3.10/site-packages -maxdepth 1 -type d -name "jaraco.context-5*" -exec rm -rf {} +
...
# Install the wheel package and constraints in a single command to enforce minimum dependency versions
RUN pip install --user --no-cache-dir "setuptools>=78.1.1" "wheel>=0.46.2" "jaraco.context>=6.1.0" "msgpack>=1.2.1" *.whl \
    && rm *.whl
