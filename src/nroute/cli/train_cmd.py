...
def _prepare_gnn_datasets(
    topo: Topology, dataset_dir: str, seed: int
) -> tuple[DataLoader, DataLoader]:
    """Generate simulation snapshots, save to Parquet, and prepare train/val DataLoaders."""
    import os
    import shutil
    ...
    from nroute.ml.training.trainer import GNNGraphDataset, collate_dataset_batch
...
def _instantiate_gnn_model(
    model_type: str, node_in_dim: int, edge_in_dim: int, hidden_dim: int
) -> torch.nn.Module:
    """Instantiate a GNN model (GCN or GraphSAGE) based on model_type."""
...
def _train_and_evaluate_gnn_model(
    ...
    """Execute GNN training across epochs and print metric reports."""
...
def _save_gnn_model(model: torch.nn.Module, model_type: str, output_dir: str) -> None:
    """Save trained GNN model to ModelStore."""
...
def train_gnn(
    ...
    train_loader, val_loader = _prepare_gnn_datasets(topo, dataset_dir, seed)
    model = _instantiate_gnn_model(model_type, node_in_dim=8, edge_in_dim=6, hidden_dim=hidden_dim)
    _train_and_evaluate_gnn_model(model, train_loader, val_loader, model_type, epochs, lr)
    _save_gnn_model(model, model_type, output_dir)
