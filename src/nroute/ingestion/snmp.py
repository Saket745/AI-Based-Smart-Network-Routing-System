...
class SNMPParser:
    ...
    def _parse_bandwidth(clean_row: dict[str, Any]) -> float:
        """Parse and convert bps to Mbps if needed."""
    ...
    def _parse_status(clean_row: dict[str, Any]) -> str:
        """Resolve link status from oper_status or ifoperstatus attributes."""
    ...
    def _calculate_utilization(
        clean_row: dict[str, Any], bandwidth: float
    ) -> tuple[float, float, float]:
        """Parse octets and calculate the utilization ratio."""
        try:
        ...
        return in_octets, out_octets, utilization
    ...
    def parse(path: str | Path) -> Topology:
        ...
        for idx, row in enumerate(raw_data):
            ...
            in_octets, out_octets, utilization = SNMPParser._calculate_utilization(
                clean_row, bandwidth
            )
