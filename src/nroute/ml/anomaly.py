...
class AnomalyDetector:
    ...
    def _predict_scores_and_flags(
        self, x_data: np.ndarray, features: pd.DataFrame
    ) -> tuple[np.ndarray, np.ndarray] | pd.DataFrame:
        """Compute anomaly scores and flags for numeric feature array across model types."""
        if self.model_type == "isolation_forest":
    ...
    @staticmethod
    def _classify_anomaly_types(features: pd.DataFrame, is_anomaly: np.ndarray) -> list[str]:
        """Classify anomaly types using heuristics on feature attributes."""
    ...
    def detect(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        res = self._predict_scores_and_flags(x_data, features)
