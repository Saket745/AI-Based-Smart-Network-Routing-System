...
class GNNEvaluator:
    ...
    def evaluate_predictions(
        ...
        if logits.size(0) > 0:
            ...
            metrics["precision"] = float(precision_score(targets, preds, zero_division=0.0))
            metrics["recall"] = float(recall_score(targets, preds, zero_division=0.0))
            metrics["f1_score"] = float(f1_score(targets, preds, zero_division=0.0))
        else:
