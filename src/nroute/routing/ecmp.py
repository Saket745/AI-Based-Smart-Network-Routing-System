...
class ECMPRouter(BaseRouter):
    ...
    def compute_path(
        ...
        **kwargs: Any,
        ...
        # Performance optimization: When flow_key is None, we only need a single
        # shortest path rather than enumerating all equal-cost paths. Directly calling
        # nx.shortest_path avoids generating the full equal-cost path tree via nx.all_shortest_paths,
        # yielding a ~9x speedup on large topologies.
        if flow_key is None:
