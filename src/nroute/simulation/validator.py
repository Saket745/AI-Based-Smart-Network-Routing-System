...
class PreFlightValidator:
    ...
    def _parse_change_patch(
        ...
        """Parse, validate, and compute SHA-256 hash for configuration change input."""
        ...
        return config_change, change_id, change_hash

    @staticmethod
    def _parse_policy_config(
        policy: PolicyGateConfig | dict[str, Any] | str | Path | None,
    ) -> tuple[PolicyGateConfig, str]:
        """Parse, validate, and compute SHA-256 hash for policy gate configuration."""
        if policy is None:
    ...
    def _evaluate_policy_rules(
        blast: BlastRadius, gate_policy: PolicyGateConfig, path_changed_ratio: float
    ) -> tuple[list[str], list[str]]:
        """Evaluate declarative policy safety rules against change blast radius."""
        ...
        return blocking_violations, warning_violations

    @staticmethod
    def _build_blast_summary_and_deltas(
        blast: BlastRadius, gate_policy: PolicyGateConfig, path_changed_ratio: float
    ) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        """Construct blast radius summary and filter critical path deltas."""
        blast_summary = {
    ...
    def validate(
        ...
        config_change, change_id, change_hash = PreFlightValidator._parse_change_patch(change)
        ...
        blocking_violations, warning_violations = PreFlightValidator._evaluate_policy_rules(
