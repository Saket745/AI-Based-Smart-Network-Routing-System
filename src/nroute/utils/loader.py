...
def _parse_import_target(import_str: str) -> tuple[str, str]:
    """Parse and validate the module path and class name from an import string."""
...
def _load_module_from_file(module_part: str, allow_unsafe: bool) -> Any:
    """Load a Python module from a local filesystem path."""
    if not allow_unsafe:
...
def _load_module_from_name(module_part: str) -> Any:
    """Load a standard Python module by its import path."""
    try:
...
def _extract_and_validate_class(
    ...
    """Retrieve class attribute from module and validate type and superclass."""
...
def load_custom_class(
    ...
    if module_part.endswith(".py") or os.path.exists(module_part):
        ...
        module = _load_module_from_name(module_part)

    return _extract_and_validate_class(module, module_part, class_name, expected_superclass)
