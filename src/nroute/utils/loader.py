...
def _parse_import_target(import_str: str) -> tuple[str, str]:
    """
    Parse and validate the import target string into module path and class name components.
    """
...
def _load_file_module(module_part: str, allow_unsafe: bool) -> Any:
    """
    Load a Python module dynamically from a local filesystem path.
    """
    if not allow_unsafe:
...
def _load_module(module_part: str, allow_unsafe: bool) -> Any:
    """
    Load a module either from a local Python file path or a standard module path.
    """
    if module_part.endswith(".py") or os.path.exists(module_part):
        return _load_file_module(module_part, allow_unsafe=allow_unsafe)
...
def _get_and_validate_class(
    module: Any, class_name: str, module_part: str, expected_superclass: type | None
) -> type:
    """
    Retrieve class attribute from module and validate type and optional inheritance constraints.
    """
...
def load_custom_class(
    ...
    module = _load_module(module_part, allow_unsafe=allow_unsafe)
    return _get_and_validate_class(module, class_name, module_part, expected_superclass)
