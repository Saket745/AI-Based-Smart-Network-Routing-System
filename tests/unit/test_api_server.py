...
def test_api_load_topology_success_cwd(client: TestClient) -> None:
    ...
    try:
        response = client.post("/api/topology/load", json={"path": str(temp_file)}, headers={"Authorization": f"Bearer {_FALLBACK_TOKEN}"})
        assert response.status_code == 200
        data = response.json()
...
def test_api_load_topology_success_temp(client: TestClient) -> None:
    ...
    try:
        topo.save(temp_path)
        response = client.post("/api/topology/load", json={"path": str(temp_path)}, headers={"Authorization": f"Bearer {_FALLBACK_TOKEN}"})
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "ok"
        assert data["nodes"] == 1
...
def test_api_load_topology_not_found(client: TestClient) -> None:
    ...
    response = client.post("/api/topology/load", json={"path": "non_existent_file_xyz.json"}, headers={"Authorization": f"Bearer {_FALLBACK_TOKEN}"})
...
def test_api_load_topology_outside_cwd_relative(client: TestClient) -> None:
    ...
    response = client.post("/api/topology/load", json={"path": "../../etc/passwd"}, headers={"Authorization": f"Bearer {_FALLBACK_TOKEN}"})
