...
def test_engine_progress_bar_usage(small_graph_data: dict[str, Any]) -> None:
    ...
    with patch("nroute.simulation.engine.Progress") as MockProgress:
        mock_instance = MockProgress.return_value
...
def test_engine_ingress_routing_failure(small_graph_data: dict[str, Any]) -> None:
    ...
    flow = FlowRecord(
        source="A", destination="D", bytes=1000, packets=10, duration=0.1, protocol="TCP", timestamp=0.0
    )
    traffic = MagicMock(spec=TrafficGenerator)
...
def test_engine_topology_exceptions(small_graph_data: dict[str, Any]) -> None:
    ...
    with patch.object(Topology, "get_edge", side_effect=mock_get_edge):
        with patch.object(Topology, "get_node", side_effect=mock_get_node):
...
def test_engine_already_completed_flow_handling(small_graph_data: dict[str, Any]) -> None:
    ...
    flow = FlowRecord(
        source="A", destination="B", bytes=1000, packets=10, duration=0.1, protocol="TCP", timestamp=0.0
    )
    traffic = MagicMock(spec=TrafficGenerator)
    traffic.generate.side_effect = lambda t, tick: [flow] if tick == 0 else []
    traffic.model = "mock"
    ...
    def callback(tick, eng):
        if tick == 0:
            ...
            eng.active_flows.append({
                "flow": FlowRecord(source="A", destination="B", bytes=0, packets=0, duration=0, protocol="TCP", timestamp=0),
                "path": ["A", "B"],
                "current_hop_idx": 1,
