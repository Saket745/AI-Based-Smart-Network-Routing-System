...
def test_plotext_renderable() -> None:
    ...
    with (
        patch(
            "nroute.visualization.live_console.plt.build",
            create=True,
            return_value="\x1b[31mRedPlot\x1b[0m",
        ),
        patch(
            "nroute.visualization.live_console.plt._build",
            create=True,
            return_value="\x1b[31mRedPlot\x1b[0m",
        ),
    ):
...
def test_live_console_error_handling() -> None:
    """Verify LiveSimulationConsole handles topology access errors gracefully."""
    topo = Topology()
    topo.add_node("A", type="router")
    topo.add_node("B", type="router")
    topo.add_edge("A", "B", bandwidth=1000, latency=5)
