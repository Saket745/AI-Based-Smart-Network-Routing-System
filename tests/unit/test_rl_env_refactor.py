...
def test_calculate_reward_basic(small_graph_data: dict[str, Any]) -> None:
    ...
    topo = _get_topo(small_graph_data)
    env = NetworkRoutingEnv(topology=topo, reward_params={
        "alpha": 1.0, "beta": 0.0, "gamma": 0.0, "delta": 0.1, "proximity": 0.0, "fairness": 0.0
    })
    env.reset(seed=42)
...
def test_calculate_reward_revisit(small_graph_data: dict[str, Any]) -> None:
    ...
    topo = _get_topo(small_graph_data)
    env = NetworkRoutingEnv(topology=topo, reward_params={
        "alpha": 0.0, "beta": 0.0, "gamma": 0.0, "delta": 0.0, "proximity": 0.0, "fairness": 0.0
    })
    env.reset(seed=42)
    env.current_node = "A"
    env.destination = "C"
